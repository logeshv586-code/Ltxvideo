...
Wan is deliberately kept at its official 480p generation size.  A final
delivery export can upscale it, but native 720p is less stable for this model.
"""
...
class WanVideoGenerator:
    ...
    def readiness(self) -> tuple[bool, str]:
        ...
        return True, "Wan2.1-T2V-1.3B ready: native 832x480, 81 frames at 16 FPS."
    ...
    def generate(self, prompt: str, seed: int | float = -1, callback: Progress = None) -> Path:
        ...
        enhanced_prompt = (
            f"{prompt.strip()}. Premium family animation, stable character design, controlled motion, "
            "clean anatomy, no text, watermark, flicker or morphing."
        )
        ...
        try:
            with torch.cuda.device(self.device_index), torch.inference_mode():
                frames = self.pipe(
                    ...
                    negative_prompt="blurry, distorted, flicker, morphing, extra limbs, text, watermark",
                    height=WAN_HEIGHT,
                    width=WAN_WIDTH,
        ...
        final = OUTPUTS_DIR / f"wan_youtube_1080p_{stamp}.mp4"
        self._report(callback, "Encoding native Wan2.1 clip", 0.90)
        export_to_video(frames, str(native), fps=WAN_FPS)
        self._report(callback, "Preparing 1080p delivery", 0.96)
        export_delivery(native, final, 1920, 1080, enhance_quality=True, target_fps=WAN_FPS)
    ...
    def generate_story(
        ...
        for index, beat in enumerate(beats):
            scene_prompt = (
                f"Story clip {index + 1} of {total}. Full story: {context}. Current event: {beat}. "
                "Preserve the same main character, costume, world, lighting and camera language."
            )
        ...
        final = OUTPUTS_DIR / f"wan_story_1080p_{stamp}.mp4"
        self._report(callback, "Combining ordered Wan2.1 clips", 0.995)
        concatenate_videos_streaming(clips, assembled, target_fps=WAN_FPS)
        export_delivery(assembled, final, 1920, 1080, enhance_quality=True, target_fps=WAN_FPS, duration_seconds=target_seconds)
        self._report(callback, f"Saved {final.name}", 1.0)
        return final, beats
